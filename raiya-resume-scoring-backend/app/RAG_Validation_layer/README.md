...
- **Where**: CLI usage:
  ```bash
  python -m src.retrieval.engine --query "Was the AI score accurate?" --mode corrective
  python -m src.retrieval.engine --query "Java Developer with 5 years" --mode standard --top-k 5
  ```
...
| `extracted_rules.json` | Pre-extracted validation rules |
| `rule_based_schema.json` | Schema definition for rule structure |
