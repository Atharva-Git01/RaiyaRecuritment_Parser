...
from app.phi4_explainer import generate_explanation, get_config
from app.schemas import CandidateFacts, JDRequirements, Role, Education, MatchDetails
from app.guardrails import validate_explanation # ensure we have this if needed explicitly, though explainer does it
...
def _map_to_facts(matcher_out: Dict, candidate_data: Dict, jd_data: Dict) -> Tuple[CandidateFacts, JDRequirements]:
    # 1. Map Facts
    roles = []
    for exp in candidate_data.get("experience", []):
        roles.append(Role(
            title=exp.get("role", "Unknown"),
            company=exp.get("company", "Unknown"),
            duration_months=0.0, # Calculation complex, set to 0 or use if available
            is_relevant=True
        ))
    ...
    for edu in candidate_data.get("education", []):
        edu_list.append(Education(
            ...
            year=edu.get("year_end") or edu.get("year_start", "Unknown")
        ))

    details = matcher_out.get("details", {})
    matched = matcher_out.get("matched_items", {})
    
    match_det = MatchDetails(
        matched_skills=matched.get("skills", {}).get("matched", []),
        missing_skills=matched.get("skills", {}).get("missing", []),
        experience_gap_years=0.0, # Placeholder
        score=matcher_out.get("final_score", 0)
    )
    
    facts = CandidateFacts(
        id=str(candidate_data.get("email", "unknown")),
        skills=candidate_data.get("skills", []),
        total_experience_years=details.get("candidate_total_experience_years", 0),
        relevant_experience_years=matcher_out.get("relevant_experience_score", 0)/10.0, # Approx
        roles=roles,
        education=edu_list,
    ...
    # 2. Map JD
    req = JDRequirements(
        jd_id=str(jd_data.get("title", "unknown")),
        title=jd_data.get("title", "Unknown"),
        must_have_skills=jd_data.get("skills_required", []),
        nice_to_have_skills=[],
        min_experience_years=float(jd_data.get("min_experience", 0))
    )
    
    return facts, req


def generate_full_report(matcher_out: Dict[str, Any], candidate_data: Dict[str, Any] = None, jd_data: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    ...
    # --- PHI-4 INTEGRATION ---
    if candidate_data and jd_data and get_config().get("enable_phi4"):
        try:
            facts, jd_req = _map_to_facts(matcher_out, candidate_data, jd_data)
            # Use candidate email/name hash as ID
            job_id_ref = f"run_{math.floor(facts.total_experience_years)}_{len(facts.skills)}"
            
            phi4_res = generate_explanation(facts, jd_req, job_id=job_id_ref)
            
            if phi4_res:
                print("✅ Phi-4 Explanation Generated")
                # Override rule-based texts with AI texts
                recruiter_text = phi4_res.summary
                
                # Merge structured fields
                structured["summary"]["short"] = phi4_res.summary
                structured["strengths"] = phi4_res.strengths # Overwrites score-based lists with semantic ones
                
                # Map weaknesses/red flags to improvements if valid
                if phi4_res.weaknesses:
                    structured["improvements"].append({"type": "ai_weakness", "why": "AI Analysis", "items": phi4_res.weaknesses})
                if phi4_res.red_flags:
                    structured["improvements"].append({"type": "ai_red_flag", "why": "Potential Inconsistency", "items": phi4_res.red_flags})
                    
        except Exception as e:
            print(f"⚠️ Phi-4 Generation Failed: {e}")
    # -------------------------
