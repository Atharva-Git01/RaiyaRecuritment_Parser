...
import matplotlib
matplotlib.use("Agg")  # Prevent GUI errors in background threads
...
def generate_batch_report(batch_id: str, batch_jobs: List[Dict[str, Any]], output_path: str) -> str:
    """
    Generate a summary PDF report for a batch of jobs.
    Includes overall progress/stats and a list of candidates.
    """
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ...
    # Calculate average score of completed jobs
    scores = []
    for j in batch_jobs:
        s = j.get("scores") or {} # Assuming pre-parsed or dict
        if isinstance(s, dict):
             # Try to get final score from main dict or nested keys
             val = s.get("final_score") or s.get("local_score", {}).get("final_score") or 0
    ...
    # Header
    story.append(BlueHeader(f"Batch Report: {batch_id}", int(avg_score)))
    story.append(Spacer(1, 16))
    
    # Stats Overview
    story.append(section_title("Execution Statistics"))
    ...
    t_stats = _make_table_from_pairs(stats_data, col_widths=[90*mm, 60*mm])
    story.append(t_stats)
    story.append(Spacer(1, 16))
    
    # Candidate List
    story.append(section_title("Candidate Execution Details"))
    
    headers = ["Job ID", "Status", "Score", "Report"]
    table_data = [headers]
    
    for j in batch_jobs:
        jid = str(j.get("job_id", "?"))
        sts = j.get("status", "unknown")
        
        # Extract score
        sc = j.get("scores") or {}
        final_sc = sc.get("final_score") or sc.get("local_score", {}).get("final_score") or 0
        
        # Link to report if available (just text for PDF)
        rpt = "Yes" if j.get("report_url") else "No"
        
        table_data.append([jid, sts, str(final_sc), rpt])
        
    # Table for candidates
    # Allow it to split across pages
    t_cands = Table(table_data, colWidths=[40*mm, 40*mm, 30*mm, 50*mm], repeatRows=1)
    
    # Style for candidate table
    style = TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0f4a7e")),
        ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
        ("ALIGN", (0, 0), (-1, -1), "LEFT"),
    ...
    t_cands.setStyle(style)
    
    story.append(t_cands)
    ...
    def footer(canvas, doc):
        ...
        canvas.setFont("Helvetica", 8)
        canvas.setFillColor(colors.grey)
        canvas.drawString(18*mm, 12*mm, f"Batch Report Generated: {batch_id}")
        canvas.drawRightString(PAGE_SIZE[0] - 18*mm, 12*mm, f"Page {doc.pageed}")
        canvas.restoreState()
