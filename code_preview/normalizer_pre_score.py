...
def normalize_for_scoring(parsed_data: dict) -> dict:
    ...
    # === 🔹 COMBINE ALL ALIASES ===
    all_aliases = {**tech_aliases, **skill_aliases, **edu_aliases}

    # === 🔹 Normalization Function ===
    def match_alias(word):
    ...
    for skill in raw_skills:
        if skill is None:
            continue
        normalized_skills.add(match_alias(str(skill)))

    # === 🔹 Normalize Education ===
    clean_education = []
    for edu in safe_data.get("education", []):
        if isinstance(edu, dict):
            degree = match_alias(str(edu.get("degree", "")))
            edu_copy = deepcopy(edu)
            edu_copy["degree"] = degree
            clean_education.append(edu_copy)
        elif edu:
            clean_education.append({"degree": match_alias(str(edu)), "institution": "", "year": ""})
